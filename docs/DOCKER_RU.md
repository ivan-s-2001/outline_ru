...
Команда **10. Создать первого администратора** запускает штатный seed-скрипт Outline. Для получения ссылки входа всё равно может потребоваться настроенная почта.
...
| `docker/windows/docker-create-admin.bat` | запускает штатный seed-скрипт |
